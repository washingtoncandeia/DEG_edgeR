...
##------------------- Análise de Zika Virus - Replicata 36 -------------------##

con_zika_36 <- makeContrasts(F_36.ZIKA - CTL.CTL, levels=design)
fit <- glmFit(dgelist, design = design)
lr_36 <- glmLRT(fit, coef=2, contrast = con_zika_36)
DE_36 <- topTags(lr_36, n=nrow(dgelist))$table
nrow(DE_36)
DE_36$AveLogCPM <- lr_36$AveLogCPM
DE_36$padj <- p.adjust(DE_36$PValue, method = "fdr", n = length(DE_36$PValue))

# Padj filter 
DE_36_padj <- subset(DE_36, padj <= 0.05)
nrow(DE_36_padj)

#logFC filter
DE_36_FC <- subset(DE_36_padj, logFC >= 1.0 | logFC <= -1.0)   # Observar se deixa 1.5
nrow(DE_36_FC)
...
write.table(as.data.frame(DE_36), sep = ",", "DE_36_ZIKV.txt", row.names = F)

#aqui eu filtro o dado pelo log do foldchange acima de 2.0 e abaixo de -2.0
DE_subset_36 <- subset(DE_36, logFC >= 1.0 | logFC <= -1.0)  
nrow(DE_subset_36)

#aqui eu estou filtrando pelo 
DE_subset_36 <- subset(DE_subset_36, padj <= 0.05)
nrow(DE_subset_36)

#separando o dataset em genes up e down regulados
DE_down_36 <- subset(DE_subset_36, logFC <= -1.0)         # Downregulated genes
DE_up_36 <- subset(DE_subset_36, logFC >= 1.0)            # Upregulated genes

## Transformar cada um numa tabela para 
## aproveitar melhor a lista de dados.

# Tabela de contagem de genes 'upregulados'
write.table(as.data.frame(DE_up_36), sep = ",", "DE_up_36_count_ZIKV_19-11-2017.txt", row.names = F)

# Tabela de contagem de genes 'downregulados'
write.table(as.data.frame(DE_down_36), sep = ",", "DE_down_36_count_ZIKV_19-11-2017.txt", row.names = F)

# MA PLOT
summary(DE_36$AveLogCPM)

pdf("36_MA_DE-19-11-2017.pdf")
with(DE_36, plot(AveLogCPM, logFC, pch=21, xlim=c(-0.5,18), axes=FALSE, main="ZIKV Sample 36 vs Control"))
axis(1, at=c(-0.5,0.5,1.55,2.55,3.5,4.55,5.5,6.5,7.55,8.5,9.55,10.5,12,14,16,18))
axis(2, at=c(-10,-8,-6,-4,-2,0,2,4,6,8,10,12))
box()
with(subset(DE_subset_36, logFC <= -1.0), points(AveLogCPM, logFC, pch=21, col="black", bg="steelblue4"))
with(subset(DE_subset_36, logFC >= 1.0), points(AveLogCPM, logFC,pch=21, col="black", bg="tomato3"))
legend("topright", legend = c("Non-DE", "Up", "Down"), pch=c(21,21,21), col="black", pt.bg = c("white","tomato3","steelblue4"))
dev.off()

# Tabela Up
#write.table(DE_up_36$genes, "36_genes_DE_UP_26-09-2017.txt", row.names = T, col.names = T)

# Tabela Down
#write.table(DE_down_36$genes, "36_genes_DE_DOWN_26-09-2017.txt", row.names = T, col.names = T)

write(DE_up_36$genes, "36_Lista_DE_UPREGULATED_novembro.txt")

# Tabela Down
write(DE_down_36$genes, "36_Lista_DE_DOWNREGULATED_novembro.txt")
