  ...
  ##------------------- Análise de Zika Virus - Replicata 48 -------------------##
  
  con_zika_48 <- makeContrasts(F_48.ZIKA - CTL.CTL, levels=design)
  fit <- glmFit(dgelist, design = design)
  lr_48 <- glmLRT(fit, coef=2, contrast = con_zika_48)
  DE_48 <- topTags(lr_48, n=nrow(dgelist))$table
  nrow(DE_48)
  DE_48$AveLogCPM <- lr_48$AveLogCPM
  DE_48$padj <- p.adjust(DE_48$PValue, method = "fdr", n = length(DE_48$PValue))
  
  # Padj filter 
  DE_48_padj <- subset(DE_48, padj <= 0.05)
  nrow(DE_48_padj)
  
  #logFC filter
  DE_48_FC <- subset(DE_48_padj, logFC >= 1.0 | logFC <= -1.0)   # Observar se deixa 1.5
  nrow(DE_48_FC)
  ...
  write.table(as.data.frame(DE_48), sep = ",", "DE_48_ZIKV.txt", row.names = F)
  
  #aqui eu filtro o dado pelo log do foldchange acima de 2.0 e abaixo de -2.0
  DE_subset_48 <- subset(DE_48, logFC >= 1.0 | logFC <= -1.0)  
  nrow(DE_subset_48)
  
  #aqui eu estou filtrando pelo 
  DE_subset_48 <- subset(DE_subset_48, padj <= 0.05)
  nrow(DE_subset_48)
  
  #separando o dataset em genes up e down regulados
  DE_down_48 <- subset(DE_subset_48, logFC <= -1.0)         # Downregulated genes
  DE_up_48 <- subset(DE_subset_48, logFC >= 1.0)            # Upregulated genes
  
  ## Transformar cada um numa tabela para 
  ## aproveitar melhor a lista de dados.
  
  # Tabela de contagem de genes 'upregulados'
  write.table(as.data.frame(DE_up_48), sep = ",", "DE_up_48_count_ZIKV_19-11-2017.txt", row.names = F)
  
  # Tabela de contagem de genes 'downregulados'
  write.table(as.data.frame(DE_down_48), sep = ",", "DE_down_48_count_ZIKV_19-11-2017.txt", row.names = F)
  
  # MA PLOT
  summary(DE_48$AveLogCPM)
  
  pdf("48_MA_DE-19-11-2017.pdf")
  with(DE_48, plot(AveLogCPM, logFC, pch=21, xlim=c(-0.5,18), axes=FALSE, main="ZIKV Sample 48 vs Control"))
  axis(1, at=c(-0.5,2.5,4.5,6.5,8.5,10.5,12.5,14.5,16.5,18))
  axis(2, at=c(-10,-8,-6,-4,-2,0,2,4,6,8,10,12))
  box()
  with(subset(DE_subset_48, logFC <= -1.0), points(AveLogCPM, logFC, pch=21, col="black", bg="steelblue4"))
  with(subset(DE_subset_48, logFC >= 1.0), points(AveLogCPM, logFC,pch=21, col="black", bg="tomato3"))
  legend("topright", legend = c("Non-DE", "Up", "Down"), pch=c(21,21,21), col="black", pt.bg = c("white","tomato3","steelblue4"))
  dev.off()
  
  # Tabela Up
  #write.table(DE_up_48$genes, "48_genes_DE_UP_26-09-2017.txt", row.names = T, col.names = T)
  
  # Tabela Down
  #write.table(DE_down_48$genes, "48_genes_DE_DOWN_26-09-2017.txt", row.names = T, col.names = T)
  
  
  write(DE_up_48$genes, "48_Lista_DE_UPREGULATED_novembro.txt")
  
  write(DE_down_48$genes, "48_Lista_DE_DOWNREGULATED_novembro.txt")
