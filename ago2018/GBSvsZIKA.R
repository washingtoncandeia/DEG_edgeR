...
library(ggplot2)
source("~/Dropbox/Rcodes/cooltable.R") #function to retrieve Up and Down genes at clusterprofiler data
...
countdata <- read.table("~/Dropbox/DualSeq_IMT/GBS/featureCounts_out.txt", header=TRUE, row.names=1, check.names = FALSE, stringsAsFactors=F)
...
countdata <- countdata %>% dplyr::select(grep("09.", names(countdata)), #GBS_rec_pair
                                         ...
                                         # grep("15.", names(countdata)), #GBS_rec_pair
...
plotPCA(rld, ntop = 500, returnData=F)
